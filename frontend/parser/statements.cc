...
namespace syntax_analyzer
{
	...
	{
		return expression{ expression::type::If, make_shared<expression>(move(condition)), { move(than_body) }};
	}

	expression
	while_stmt(expression&& condition, expression&& loop_body)
	{
		return expression{ expression::type::While, make_shared<expression>(move(condition)), { move(loop_body) }};
	}
