...
expression::expression (std::string &&str)
    : m_terminal_value{ str }, m_type{ expression::type::String }, m_peculiar{}
{
}

expression::expression (int num)
    : m_terminal_value{ num }, m_type{ expression::type::Number }, m_peculiar{}
{
}
...
void
expression::merge_with (expression &&other)
{
...
expression::expression (expression::type type,
    ...
    : m_type{ type }, m_operands (move (operands)), m_peculiar{}
{
}

expression::expression (expression::type type,
			expression::peculiar_type &&peculiar,
			std::vector<expression> operands)
    : m_type{ type }, m_peculiar{ move (peculiar) }, m_operands{ move (
							 operands) }
{
}
...
expression::operator== (const expression &other) const noexcept
{
    ...
    if (peculiar_value && *peculiar ().value () != *other.peculiar ().value ())
	return false;
