...
namespace cli
{
	...
	struct settings
	{
		...
		enum class print_stage_enum
		{
			ast, ir, asm_, all, none
		};
	...
	bool print_ast()
	{
		using se = settings::print_stage_enum;
		return globals.print_stage == se::ast || globals.print_stage == se::all;
	}

	bool print_asm()
	{
		using se = settings::print_stage_enum;
		return globals.print_stage == se::asm_ || globals.print_stage == se::all;
	}

	bool print_ir()
	{
		using se = settings::print_stage_enum;
		return globals.print_stage == se::ir || globals.print_stage == se::all;
	}
	...
	parse_arguments(int argc, const char* argv[]) /* throws */
	{
		...
		if (parser.exists("pprint"))
		{
			...
			if (s == "asm") globals.print_stage = settings::print_stage_enum::asm_;
			else if (s == "all") globals.print_stage = settings::print_stage_enum::all;
			else if (s == "none") globals.print_stage = settings::print_stage_enum::none;
			else if (s == "ir") globals.print_stage = settings::print_stage_enum::ir;
			else if (s == "ast") globals.print_stage = settings::print_stage_enum::ast;
		}
