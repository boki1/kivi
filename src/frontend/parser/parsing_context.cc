...
{
parsing_context::parsing_context(const char *code_beginning, std::string *filename) : lexer_cursor(code_beginning)
{
...
expression parsing_context::define_identifier(const identifier &ident)
{
	LOG_S(INFO) << "Defining identifier \"" << ident.name() << "\"";

	bool is_duplicate = std::any_of(scopes().begin(), scopes().end(), [&](const identifier &s) { return s == ident; });
	...
	using itype = identifier::type;
	switch (ident.get_type())
	{
		case itype::Function:	m_current_function.set_name(ident.name()); break;
	}

	return expression(ident);
}

expression parsing_context::use_identifier(const std::string &name) const
{
	LOG_S(INFO) << "Using identifier \"" + name << "\".";

	auto it = std::find_if(scopes().begin(), scopes().end(), [&](const auto &s) { return s.name() == name; });
...
const function &parsing_context::define_function_body(const std::string &name, expression &&body)
{
	/// Adds implicit return statement at the end of the block
	/// "concatenated" by this double-compound statement
	auto new_body = sa::compound_stmt({body, return_stmt()});
...
expression parsing_context::define_local(std::string &&name)
{
...
expression parsing_context::define_function(std::string name)
{
	LOG_S(INFO) << "Defining function (\"" + name << "\")" << '\n';
	auto appears = std::any_of(functions().begin(), functions().end(), [&](const auto &fun) { return fun.name() == name; });

	if (appears)
		throw kp::syntax_error(yy_location, "Duplicate definition of \"" + name + "\".");

	return define_identifier(identifier(identifier::type::Function, move(name), functions().size()));
}

expression parsing_context::define_parameter(std::string &&name)
{
...
expression parsing_context::define_register()
{
